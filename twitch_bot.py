...
class TwitchBot(Chatbot):
    ...
    def __init__(
        self, logs_folder, log_to_console=True, pokemon_exception_handling=False
    ):
    ...
    def connect_and_join(self, password, nickname, channel):
        ...
        self.setup_logging(
            self.logs_folder, f"TwitchBot.{self.channel}.{self.nickname}", True
        )
    ...
    def send_supported_commands(self, cmd: Chatbot.Command):
        if cmd.params is None:
            ...
            help_commands_list = " or ".join(
                ["!" + e for e in TwitchBot.help_command_names]
            )
            self.send_privmsg(
                f"Sending {help_commands_list} followed by a command gives usage detail."
            )
        else:
    ...
    def send_privmsg(self, msg):
        ...
        if text_length > TwitchBot.twitch_text_len_max:
            self.logger.warning(
                f"Message length {text_length} > {TwitchBot.twitch_text_len_max}"
            )
        return sent_msg, num_sent_bytes

    def register_command(self, command_name, callback):
        copied_callback = functools.partial(callback)
        copied_callback.__doc__ = TwitchBot.format_usage_docstring(
            command_name, callback.__doc__
        )
