...
class Message:
    ...
    def from_str(cls, msg_str):
        ...
        if msg.startswith(":"):
            ...
            return cls(
                msg_str, parts[0][1:], parts[1], parts[2] if len(parts) == 3 else None
            )
        else:
...
class Client:
    ...
    def send(self, msg_prefix, msg_command, command_params, newline_replacement=" "):
        msg_str = self.format_msg(
            msg_prefix, msg_command, command_params, newline_replacement
        )
        num_sent_bytes = self.sock.send(bytes("\r\n" + msg_str + "\r\n", "UTF-8"))
        return msg_str, num_sent_bytes

    def format_msg(
        self, msg_prefix, msg_command, command_params, newline_replacement=" "
    ):
