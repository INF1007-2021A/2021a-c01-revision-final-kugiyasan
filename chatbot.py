...
class Chatbot:
    ...
    def connect(self, host, port, use_ssl=False):
        ...
        if use_ssl:
            self.logger.debug(
                f"Connected using SSL version '{self.irc_client.sock.version()}'"
            )
        else:
    ...
    def send(self, command, params):
        sent_msg, num_sent_bytes = self.irc_client.send(
            self.global_prefix, command, params
        )
        self.logger.info("< " + sent_msg)
    ...
    def is_known_command(self, command_str):
        return (
            command_str[0] == self.command_char
            and command_str[1:] in self.command_methods
        )
